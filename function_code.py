...
# def remove_tabl():
#     db.connect()
#     amount = len(KD.select())
#     for i in range(amount):
#         (KD.get(KD.id == (i + 1))).delete_instance()
#     db.close()
# # Экспорт таблицы KD
# def export_kzfkp(path_kzfkp, uso, number_row, flread_exel, flread_hat, export_tabl, name_column):
#     tabl     = []
#     hat_tabl = []
#     hat_num  = {}
#     wb = openpyxl.load_workbook(path_kzfkp, read_only=True, data_only=True)
#     if flread_exel:
#         for sheet in wb.worksheets:
#             tabl.append(sheet.title)
#         wb.close()
#         return tabl
#     if flread_hat:
#         for sheet in wb.worksheets:
...
#                     data.append(array)
#         wb.close()
#         # Откроем содинение с базой и если надо создадим таблицу
#         try:
#             db.connect()
#             KD.create_table()
...
#
#             if name    is None: continue
#             if rack    is None: continue
#             if modul   is None: continue
#             if channel is None: continue
#             if (kont is None) or (kont == 'None'): kont = ''
#
#             # Проверяем на существование файл
#             exists = KD.select().where((KD.Cabinet == uso)   &
#                                        (KD.Basket  == rack)  &
#                                        (KD.Module  == modul) &
#                                        (KD.Сhannel == channel))
#
#             if bool(exists):
#                 for kd in KD.select():
#                     if (kd.Cabinet == f'{uso}')   and \
#                        (kd.Basket  == f'{rack}')  and \
#                        (kd.Module  == f'{modul}') and \
#                        (kd.Сhannel == f'{channel}'):
#
#                         if kd.Tag         != tag:    note = f'{kd.Tag}'
#                         if kd.Description != name:   note = f'{note};{kd.Description};'
#                         if kd.Scheme      != schema: note = f'{note};{kd.Scheme};'
#                         if kd.KlK         != klk:    note = f'{note};{kd.KlK};'
#                         if kd.Сontact     != kont:   note = f'{note};{kd.Сontact};'
#                         break
#                 KD.update(
#                     Tag         = tag,
#                     Description = name,
...
#                     Сhannel     = channel,
#                     Note        = note
#                 ).where((KD.Cabinet == uso)   &
#                         (KD.Basket  == rack)  &
#                         (KD.Module  == modul) &
#                         (KD.Сhannel == channel)).execute()
#
#                 if note == '':
#                     logger.info(f'Таблица KD. Шкаф {uso}. {name}. Изменения отсутствуют!')
#                 else:
#                     logger.error(f'Таблица KD. Шкаф {uso}. {name}. Изменения до: {note}')
#             else:
#                 KD.create(
#                     Cabinet     = uso,
#                     Tag         = tag,
...
#                     Сhannel     = channel
#                 )
#                 logger.info(f'Таблица KD. Шкаф {uso}. {name}. Новый сигнал')
...
#         if (kd.Cabinet != uso):
#             uso = kd.Cabinet
#         if (kd.Cabinet == uso) and (kd.Basket != rack):
#             rack = kd.Basket
#         if (kd.Cabinet == uso) and (kd.Basket == rack) and (kd.Module != module):
#             module = kd.Module
#             data.append(dict(uso    = uso,
#                              rack   = rack,
#                              module = module))
#
#
